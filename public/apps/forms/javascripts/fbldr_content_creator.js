...
jive.fbldr.ContentCreator = function(template, form) {
    ...
    var getFieldValue = function(field) {
        ...
        if (field.type == "boolean") {
            if ($j(form).find("#fbldr-field-" + field.id).is(":checked")) {
                value = '<img class="jive_macro jive_macro_emoticon jive_emote" src="/images/emoticons/check.png" jivemacro="emoticon" ___jive_emoticon_name="check" alt="CHECKED"/>';
            } else {
                value = "";
            }
    ...
    var doActions = function(content, callback) {
        if (jive.fbldr.isVer3()) {
            osapi.jive.corev3.metadata.getPropertyMetadata({name:"instance.url"}).execute(function(response) {
                // console.log('Using v3 base URL: ' + response.content.value);
                content.href = response.content.value + content.href;
